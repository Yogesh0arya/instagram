...
function Post({id, username, userImg, img, caption}) {
    ...
    // store the comments to firebase
    const sendComment = async (e) => {
        e.preventDefault()

        // make a copy of a comment for front end it should be super responsive
        const commentToSend = comment
        setComment('')

        // going to db then posts then id then collection of comments
    ...
    // store the likes to firebase
    const likePost = async (e) => {
        if (hasLiked){
            await deleteDoc(doc(db, 'posts', id , 'likes', session.user.uid));
        } else{
            // likes id should be the user id
